...
const Home = () => {
  ...
  const resumeCheckRef = useRef(null);
  const resumeTailoringRef = useRef(null);
  const isResumeCheckInView = useInView(resumeCheckRef, { once: true });
  const isResumeTailoringInView = useInView(resumeTailoringRef, { once: true });
  ...
  return (
    ...
    >
      ...
      {/* Hero Section */}
      <div className="flex flex-col md:flex-row items-center justify-center px-4 md:px-12 py-10 md:py-20">
        {/* Left: Large Image */}
        <motion.div className="w-full md:w-1/2 mb-6 md:mb-0" whileHover={{ scale: 1.05 }} transition={{ duration: 0.3 }}>
          <img src={heroImage} alt="Hero" className="rounded-lg shadow-lg w-full md:w-auto" />
        </motion.div>

        {/* Right: Text Content */}
        <motion.div
          className="w-full md:w-1/2 md:pl-10 bg-black bg-opacity-70 p-6 md:p-8 rounded-lg"
          initial={{ opacity: 0, x: 50 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 1 }}
        >
          <h2 className="text-4xl md:text-5xl font-bold text-primary">Craft Your Success</h2>
          <p className="mt-4 text-lg">
            Build professional resumes, cover letters, and portfolios with ease. 
            Transform your career with expert guidance.
          </p>
      ...
      {/* ATS Optimization Section */}
      <div className="flex flex-col md:flex-row items-center justify-between px-4 md:px-12 py-10 md:py-20">
        {/* Left: Text */}
        <motion.div
          className="w-full md:w-1/2 bg-black bg-opacity-100 p-6 md:p-8 rounded-lg"
          initial={{ opacity: 0, x: -50 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 1 }}
        >
        ...
        {/* Right: Image with Hover Effect */}
        <motion.div className="w-full md:w-1/2 flex justify-center md:justify-end mt-6 md:mt-0">
          <motion.img
            src={atsImage}
            alt="ATS Optimization"
      ...
      {/* Check Your Resume Section */}
      <div ref={resumeCheckRef} className="flex flex-col md:flex-row items-center justify-between px-4 md:px-12 py-10 md:py-20">
        {/* Left: Text Section with Animation */}
        <motion.div
          className="w-full md:w-1/2 bg-black bg-opacity-100 p-6 md:p-8 rounded-lg"
          initial={{ opacity: 0, x: -50 }}
          animate={isResumeCheckInView ? { opacity: 1, x: 0 } : {}}
          transition={{ duration: 1 }}
        >
          <h2 className="text-3xl md:text-4xl font-bold text-primary">
            Check your resume for grammatical and punctuation errors
      ...
      {/* Resume Tailoring Section */}
      <div ref={resumeTailoringRef} className="flex flex-col md:flex-row items-center justify-between px-4 md:px-12 py-10 md:py-20">
        {/* Left: Text Section with Background */}
        <motion.div
          className="w-full md:w-1/2 bg-black bg-opacity-100 p-6 md:p-8 rounded-lg"
          initial={{ opacity: 0, x: -50 }}
          animate={isResumeTailoringInView ? { opacity: 1, x: 0 } : {}}
          transition={{ duration: 1 }}
        >
          ...
          <ul className="mt-4 space-y-2 text-lg md:text-2xl">
            <li>✅ Skills and experience section analysis</li>
            <li>✅ Actionable checklist of what else to add to your resume</li>
            <li>✅ Instant comparison between your resume and the job posting</li>
          </ul>
